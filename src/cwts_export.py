...
bibliographic coupling), then upload CWTS tables to BigQuery.

All network artefacts are uploaded to BigQuery only (no lasting local data files).
Cluster assignment is handled separately by subprocess_leiden.py.

BigQuery (ocean-tech-adv-analytics-c-tfs.raw_citation_network_data):
    pubs_raw_{timestamp}
    pub_metadata_raw_{timestamp}
    cit_links_raw_{timestamp}
    run_metadata_{timestamp}
...
TOP_N_JOURNALS = int(
    os.environ.get("TOP_N_JOURNALS", "5")
)  # these ones are test journals
...
LOG_DIR = Path(r"C:\Users\sophie.wilson\Documents\scope_drift_outputs\logs\cwts_output")
...
# ---------------------------------------------------------------------------
# Step 6: Upload CWTS tables to BigQuery (no lasting local data files)
# ---------------------------------------------------------------------------
def write_cwts_files(
    ...
    """
    Build CWTS tables and upload them to BigQuery.
    ...
    log.info(f"Building CWTS tables for BigQuery  ({len(sorted_pids):,} nodes)...")
    ...
    log.info(f"  pubs            : {len(pubs_df):,} rows  ({n_core:,} Frontiers)")
    ...
    log.info(f"  pub_metadata    : {len(pub_metadata_df):,} rows")
    ...
    log.info(
        f"  cit_links       : {len(cit_links_df):,} rows  ({len(df):,} unique × 2 directions)"
    )
...
def main():
    try:
        ...
        # Step 6: Upload tables to BigQuery (no lasting local data files)

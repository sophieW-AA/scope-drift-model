"""
Join Neurorobotics scope-drift papers to section + Research Topic via taxonomy.

Canonical RDM path (article → taxonomy / research_topic):
  ...
  SELECT
    ...
    t.journal,
    t.section,
    t.type AS taxonomy_type,  -- Field Journal / Specialty Journal / Specialty Section
    rt.research_topic_id,
    rt.title AS research_topic_title
  FROM reporting_data_mart.article a
  LEFT JOIN reporting_data_mart.taxonomy t ON a.taxonomy_id = t.taxonomy_id
  LEFT JOIN reporting_data_mart.research_topic rt
...
Usage:
  ...
  python src/scope_drift/probe_sections_rts.py --journal "Frontiers in Earth Science"
"""
...
from journal_profile import journal_dir, journal_short  # noqa: E402
from neuro_analysis import JOURNAL as DEFAULT_JOURNAL, get_journal, load_dashboards  # noqa: E402
...
def probe_journal(journal: str, outdir: Path) -> Path:
    ...
    int_ids = list(scope_by_id)
    if not int_ids:
        raise SystemExit(f"No scatter papers for {journal}")
    print(f"{journal}: scatter papers {len(int_ids)}")
    outdir.mkdir(parents=True, exist_ok=True)

    client = bigquery.Client(project=BQ_PROJECT, location="EU")
    ids_sql = ",".join(str(i) for i in int_ids)

    # Journal-level taxonomy inventory
    inv = client.query(
        """
        SELECT
    ...
    print("=== Taxonomy inventory ===")
    print(inv.to_string(index=False))

    # Run papers → RDM via title → taxonomy + research_topic
    q = f"""
    WITH run AS (
      SELECT
        m.int_id,
    ...
    linked AS (
      SELECT
        ...
        rt.research_topic_id,
        rt.title AS research_topic_title,
        DATE(COALESCE(rt.create_date, rt.research_topic_create_date)) AS rt_create_date,
        DATE(rt.online_date) AS rt_online_date,
    ...
    SELECT
      ...
      l.section_id,
      l.taxonomy_type,
      l.section_publish_date,
      l.taxonomy_create_date,
      l.journal_launch_date,
      l.research_topic_id,
    ...
    print(f"Join rows before dedupe: {len(df)}")
    ...
    df.to_parquet(out_file(outdir, "section_rt_join.parquet"), index=False)
    print(f"matched={int(df['matched'].sum())}/{len(df)}")
    print("taxonomy_type:", df["taxonomy_type"].fillna("(none)").value_counts().to_dict())
    ...
    if df["has_section"].any():
        by_sec = (
            df.groupby(
                ["section", "taxonomy_type", "section_publish_date", "taxonomy_create_date"],
                dropna=False,
            )
        ...
        by_sec.to_csv(out_file(outdir, "oos_by_section_taxonomy.csv"), index=False)
    ...
    yr = (
        df.groupby(["airak_year", "in_rt"])
        .agg(n=("int_id", "count"), oos_n=("is_oos", "sum"))
        .reset_index()
    )
    yr["oos_pct"] = (100 * yr["oos_n"] / yr["n"]).round(1)
    yr.to_csv(out_file(outdir, "oos_by_year_rt.csv"), index=False)

    rt = df[df["in_rt"]].copy()
    if rt.empty:
        pd.DataFrame(columns=["rt_launch_year", "n_articles", "n_rts", "oos_n", "oos_pct"]).to_csv(
            out_file(outdir, "rt_launch_cohorts.csv"), index=False
        )
        pd.DataFrame(
            columns=["research_topic_id", "research_topic_title", "rt_launch_year", "n", "oos_n", "oos_pct"]
        ).to_csv(out_file(outdir, "rt_oos_by_topic.csv"), index=False)
    else:
        by_launch = (
            rt.groupby("rt_launch_year")
            .agg(n_articles=("int_id", "count"), n_rts=("research_topic_id", "nunique"), oos_n=("is_oos", "sum"))
            .reset_index()
            .sort_values("rt_launch_year")
        )
        by_launch["oos_pct"] = (100 * by_launch["oos_n"] / by_launch["n_articles"]).round(1)
        by_launch.to_csv(out_file(outdir, "rt_launch_cohorts.csv"), index=False)
        print("\nArticles by RT create year:")
        print(by_launch.to_string(index=False))
        ...
        by_rt = by_rt.sort_values(["oos_n", "n"], ascending=False)
        by_rt.to_csv(out_file(outdir, "rt_oos_by_topic.csv"), index=False)
        print("\nTop RTs by OOS count:")
        print(by_rt.head(15).to_string(index=False))

    n_sec = int(df["has_section"].sum())
    types = df["taxonomy_type"].fillna("(none)").value_counts().to_dict()
    if n_sec == 0:
        verdict = (
            f"Via taxonomy join: {journal_short(journal)} papers have no Specialty Section rows. "
            "Section launch cannot explain drift. Use Research Topics as the structural launch unit."
        )
    else:
        verdict = (
            f"{journal_short(journal)} has Specialty Sections covering {n_sec} of {len(df)} run papers. "
            "Compare section launch dates against the drift onset before attributing the shift."
        )
    summary = {
        "join_path": "article.taxonomy_id→taxonomy; article_research_topic_id→research_topic",
        "journal": journal,
        "n_scatter": len(int_ids),
        "n_matched_rdm": int(df["matched"].sum()),
        "n_with_section": n_sec,
        ...
        "oos_pct_non_rt": round(100 * float(df.loc[~df["in_rt"], "is_oos"].mean()), 1)
        if (~df["in_rt"]).any()
        else None,
        "verdict": verdict,
        "sections": df["section"].fillna("(none)").value_counts().head(20).to_dict(),
    }
    summary_path = out_file(outdir, "section_rt_summary.json")
...
def main() -> None:
    ...
    probe_journal(args.journal, outdir)

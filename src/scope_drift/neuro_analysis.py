...
Reads dashboard HTML from scope_drift_outputs/output (run 20260721_122750).
"""
...
from paths import DASHBOARDS, WORK_DIR
...
def contested_oos_titles(
    ...
    for p in maps_journal.get("scatter") or []:
        if p.get("s") != 0:
            continue
        if year is not None and p.get("yr") != year:
            continue
        title = p.get("t") or ""
        low = title.lower()
...
def load_section_rt_summary() -> dict:
    ...
    path = WORK_DIR / "neurorobotics_section_rt_summary.json"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing {path.name}. Run: python src/scope_drift/probe_sections_rts.py"
        )

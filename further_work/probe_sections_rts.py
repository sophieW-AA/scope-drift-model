"""
Join Neurorobotics scope-drift papers to section + Research Topic via taxonomy.

Canonical RDM path (article → taxonomy / research_topic):

  SELECT
    a.article_id,
    a.article_title AS article_title,
    t.journal,
    t.section,
    t.type AS taxonomy_type,  -- Field Journal / Specialty Journal / Specialty Section
    rt.research_topic_id,
    rt.title AS research_topic_title
  FROM reporting_data_mart.article a
  LEFT JOIN reporting_data_mart.taxonomy t ON a.taxonomy_id = t.taxonomy_id
  LEFT JOIN reporting_data_mart.research_topic rt
    ON a.article_research_topic_id = rt.research_topic_id
  WHERE a.space_id = 1 AND a.is_deleted = FALSE

Usage:
  python further_work/probe_sections_rts.py
"""
from __future__ import annotations

import json
import sys
from pathlib import Path

import pandas as pd
from google.cloud import bigquery

HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(HERE))

from neuro_analysis import JOURNAL, get_journal, load_dashboards  # noqa: E402

RUN = "20260721_122750"
BQ_PROJECT = "ocean-tech-adv-analytics-c-tfs"
TBL_META = f"{BQ_PROJECT}.raw_citation_network_data.pub_metadata_raw_{RUN}"


def main() -> None:
    _, _, maps = load_dashboards()
    mj = get_journal(maps)
    scatter = mj.get("scatter") or []
    scope_by_id = {int(p["i"]): int(p.get("s", 1)) for p in scatter if p.get("i") is not None}
    int_ids = list(scope_by_id)
    print(f"Scatter papers: {len(int_ids)}")

    client = bigquery.Client(project=BQ_PROJECT, location="EU")
    ids_sql = ",".join(str(i) for i in int_ids)

    # Journal-level taxonomy inventory
    inv = client.query(
        """
        SELECT
          t.type AS taxonomy_type,
          t.section,
          t.section_id,
          DATE(t.publish_date) AS section_publish_date,
          DATE(t.journal_launch_date) AS journal_launch_date,
          DATE(t.create_date) AS taxonomy_create_date,
          COUNT(*) AS n_articles
        FROM `ocean-breeze-tier-1.reporting_data_mart.article` a
        LEFT JOIN `ocean-breeze-tier-1.reporting_data_mart.taxonomy` t
          ON a.taxonomy_id = t.taxonomy_id
        WHERE a.space_id = 1
          AND a.is_deleted = FALSE
          AND a.is_published = TRUE
          AND LOWER(t.journal) = LOWER(@journal)
          AND EXTRACT(YEAR FROM COALESCE(a.first_publish_date, a.stage_date_published, a.create_date))
              BETWEEN 2020 AND 2026
        GROUP BY 1, 2, 3, 4, 5, 6
        ORDER BY n_articles DESC
        """,
        job_config=bigquery.QueryJobConfig(
            query_parameters=[bigquery.ScalarQueryParameter("journal", "STRING", JOURNAL)]
        ),
    ).to_dataframe()
    print("=== Taxonomy inventory ===")
    print(inv.to_string(index=False))

    # Run papers → RDM via title → taxonomy + research_topic
    q = f"""
    WITH run AS (
      SELECT
        m.int_id,
        m.pub_id,
        m.title,
        EXTRACT(YEAR FROM SAFE.PARSE_DATE('%Y-%m-%d', SUBSTR(CAST(m.date AS STRING), 1, 10))) AS airak_year,
        LOWER(TRIM(REGEXP_REPLACE(m.title, r'\\s+', ' '))) AS title_norm
      FROM `{TBL_META}` m
      WHERE m.int_id IN ({ids_sql})
        AND LOWER(m.journal) = LOWER(@journal)
    ),
    linked AS (
      SELECT
        a.article_id,
        a.article_title,
        t.journal,
        t.section,
        t.section_id,
        t.type AS taxonomy_type,
        DATE(t.publish_date) AS section_publish_date,
        DATE(t.create_date) AS taxonomy_create_date,
        DATE(t.journal_launch_date) AS journal_launch_date,
        rt.research_topic_id,
        rt.title AS research_topic_title,
        DATE(COALESCE(rt.create_date, rt.research_topic_create_date)) AS rt_create_date,
        DATE(rt.online_date) AS rt_online_date,
        DATE(COALESCE(a.first_publish_date, a.stage_date_published)) AS article_pub_date,
        LOWER(TRIM(REGEXP_REPLACE(a.article_title, r'\\s+', ' '))) AS title_norm
      FROM `ocean-breeze-tier-1.reporting_data_mart.article` a
      LEFT JOIN `ocean-breeze-tier-1.reporting_data_mart.taxonomy` t
        ON a.taxonomy_id = t.taxonomy_id
      LEFT JOIN `ocean-breeze-tier-1.reporting_data_mart.research_topic` rt
        ON a.article_research_topic_id = rt.research_topic_id
      WHERE a.space_id = 1
        AND a.is_deleted = FALSE
        AND a.is_published = TRUE
        AND LOWER(t.journal) = LOWER(@journal)
    )
    SELECT
      r.int_id,
      r.pub_id,
      r.title,
      r.airak_year,
      l.article_id,
      l.journal,
      l.section,
      l.section_id,
      l.taxonomy_type,
      l.section_publish_date,
      l.taxonomy_create_date,
      l.journal_launch_date,
      l.research_topic_id,
      l.research_topic_title,
      l.rt_create_date,
      l.rt_online_date,
      l.article_pub_date
    FROM run r
    LEFT JOIN linked l ON r.title_norm = l.title_norm
    """
    df = client.query(
        q,
        job_config=bigquery.QueryJobConfig(
            query_parameters=[bigquery.ScalarQueryParameter("journal", "STRING", JOURNAL)]
        ),
    ).to_dataframe()

    print(f"Join rows before dedupe: {len(df)}")
    df["matched"] = df["article_id"].notna()
    df = df.sort_values(["int_id", "matched"], ascending=[True, False]).drop_duplicates("int_id")
    df["is_oos"] = df["int_id"].map(scope_by_id) == 0
    df["in_rt"] = df["research_topic_id"].notna()
    df["has_section"] = df["section"].notna() & (df["section"].astype(str).str.strip() != "")
    df["rt_launch_year"] = pd.to_datetime(df["rt_create_date"], errors="coerce").dt.year

    df.to_parquet(HERE / "neurorobotics_section_rt_join.parquet", index=False)
    print(f"matched={int(df['matched'].sum())}/{len(df)}")
    print("taxonomy_type:", df["taxonomy_type"].fillna("(none)").value_counts().to_dict())
    print("with section:", int(df["has_section"].sum()))
    print("with RT:", int(df["in_rt"].sum()))

    if df["has_section"].any():
        by_sec = (
            df.groupby(
                ["section", "taxonomy_type", "section_publish_date", "taxonomy_create_date"],
                dropna=False,
            )
            .agg(n=("int_id", "count"), oos_n=("is_oos", "sum"))
            .reset_index()
        )
        by_sec["oos_pct"] = (100 * by_sec["oos_n"] / by_sec["n"]).round(1)
        by_sec = by_sec.sort_values("n", ascending=False)
        print("\n=== OOS by section ===")
        print(by_sec.to_string(index=False))
        by_sec.to_csv(HERE / "neurorobotics_oos_by_section_taxonomy.csv", index=False)

    cmp = df.groupby("in_rt").agg(n=("int_id", "count"), oos_n=("is_oos", "sum"))
    cmp["oos_pct"] = (100 * cmp["oos_n"] / cmp["n"]).round(1)
    print("\nRT vs non-RT OOS:")
    print(cmp.to_string())

    yr = (
        df.groupby(["airak_year", "in_rt"])
        .agg(n=("int_id", "count"), oos_n=("is_oos", "sum"))
        .reset_index()
    )
    yr["oos_pct"] = (100 * yr["oos_n"] / yr["n"]).round(1)
    yr.to_csv(HERE / "neurorobotics_oos_by_year_rt.csv", index=False)

    rt = df[df["in_rt"]].copy()
    by_launch = (
        rt.groupby("rt_launch_year")
        .agg(n_articles=("int_id", "count"), n_rts=("research_topic_id", "nunique"), oos_n=("is_oos", "sum"))
        .reset_index()
        .sort_values("rt_launch_year")
    )
    by_launch["oos_pct"] = (100 * by_launch["oos_n"] / by_launch["n_articles"]).round(1)
    by_launch.to_csv(HERE / "neurorobotics_rt_launch_cohorts.csv", index=False)
    print("\nArticles by RT create year:")
    print(by_launch.to_string(index=False))

    by_rt = (
        rt.groupby(["research_topic_id", "research_topic_title", "rt_launch_year"], dropna=False)
        .agg(n=("int_id", "count"), oos_n=("is_oos", "sum"))
        .reset_index()
    )
    by_rt["oos_pct"] = (100 * by_rt["oos_n"] / by_rt["n"]).round(1)
    by_rt = by_rt.sort_values(["oos_n", "n"], ascending=False)
    by_rt.to_csv(HERE / "neurorobotics_rt_oos_by_topic.csv", index=False)
    print("\nTop RTs by OOS count:")
    print(by_rt.head(15).to_string(index=False))

    summary = {
        "join_path": "article.taxonomy_id→taxonomy; article_research_topic_id→research_topic",
        "journal": JOURNAL,
        "n_scatter": len(int_ids),
        "n_matched_rdm": int(df["matched"].sum()),
        "n_with_section": int(df["has_section"].sum()),
        "n_with_rt": int(df["in_rt"].sum()),
        "taxonomy_types": df["taxonomy_type"].fillna("(none)").value_counts().to_dict(),
        "oos_pct_overall": round(100 * float(df["is_oos"].mean()), 1),
        "oos_pct_rt": round(100 * float(df.loc[df["in_rt"], "is_oos"].mean()), 1)
        if df["in_rt"].any()
        else None,
        "oos_pct_non_rt": round(100 * float(df.loc[~df["in_rt"], "is_oos"].mean()), 1)
        if (~df["in_rt"]).any()
        else None,
        "verdict": (
            "Via taxonomy join: all Neurorobotics papers are taxonomy_type='Specialty Journal' "
            "with null section — no Specialty Section rows. Section launch cannot explain drift. "
            "Use Research Topics (article_research_topic_id) as the structural launch unit."
        ),
    }
    (HERE / "neurorobotics_section_rt_summary.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
    print("\nSUMMARY")
    print(json.dumps(summary, indent=2))


if __name__ == "__main__":
    main()

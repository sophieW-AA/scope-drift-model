...
def build_graph(df_edges: pd.DataFrame):
    ...
    # Vectorized edge mapping (much faster than iterrows for 1.7M edges)
    log.info(f"Mapping edges to node indices...")
    src_mapped = df_edges["src"].map(id_map)
    tgt_mapped = df_edges["tgt"].map(id_map)
    
    # Filter out any unmapped edges and create tuples
    ...
    while iteration < max_iterations:
        ...
        for edge in G.es:
            ...
            if src_comm in small_comms:
                small_comm_connections[src_comm][tgt_comm] += 1
            if tgt_comm in small_comms:
                small_comm_connections[tgt_comm][src_comm] += 1
...
def main():
    ...
    if NETWORK_MODE == "full":
        ...
        df_edges, final_nodes = get_full_network_edges(frontiers_pub_ids, journal_ids)
    else:
